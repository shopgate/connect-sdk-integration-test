...
intercept((chunk) => {
  ...
  for (const propertyName in configSchema.properties) {
    const property = configSchema.properties[propertyName]
    if (property.format === 'string') {
      const properyValue = config.get(propertyName)
      filtered = filtered.replace(properyValue, `**${property.env || propertyName}**`)
    }
  }
  return filtered
})
...
class IntegrationTestUtils {
  ...
  get executable () {
    return config.get('executable').replace(/\\/g, '\\\\')
  }
  ...
  async terminate (proc) {
    ...
    const pid = proc instanceof ChildProcess ? proc.pid : proc

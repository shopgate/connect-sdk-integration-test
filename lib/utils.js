...
class IntegrationTestUtils {
  ...
  async cleanup () {
    if (this._backendProcess) {
      if (this._backendProcess.killed) {
        delete this._backendProcess
      } else {
        await new Promise((resolve, reject) => {
          this._backendProcess.on('close', resolve)
          this._backendProcess.on('error', reject)
          this._backendProcess.kill('SIGINT')
        })
      }
    }
  ...
  get runner () {
    const runner = nixt({ colors: false }).cwd(this.appDir)

    Object.keys(this.env).forEach((key) => {
      runner.env(key, this.env[key])
    })

    const end = runner.end.bind(runner)
    runner.end = function (cb) {
      if (cb) return end(cb)
      return promisify(end)()
    }
